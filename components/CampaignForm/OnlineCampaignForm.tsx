...
import {z} from "zod";
import useOnlineCampaignForm from "./hooks/useOnlineCampaignForm";
...
const OnlineCampaignForm: React.FC<Props> = ({action}) => {
    ...
    const FormSchema = BaseOnlineCampaignSchema
    ...
    const {fields: campaignGroupFields, append: appendGroup, remove: removeGroup} = useFieldArray({
        control,
        name: "campaignGroups",
        shouldUnregister: true,
    });

    const {fields: campaignLevelFields, append: appendLevel, remove: removeLevel} = useFieldArray({
        control,
        name: "campaignLevels",
        shouldUnregister: true,
    });
    ...
    const selectedGroups: IGroup[] = watch('campaignGroups')?.map((group) => {
        return {
            id: group.groupId,
    ...
    const selectedLevels: ILevel[] = watch('campaignLevels')?.map((level) => {
        return {
            id: level.levelId,
                        ...
                        <div>
                            ...
                            <SelectGroupsTable
                                ...
                                handleRemoveGroup={(group) => {
                                    const index = campaignGroupFields.findIndex((g) => g.groupId === group.id);
                                    if (index >= 0) {
                                        removeGroup(index);
                                    }
                                }}
                        ...
                        <div>
                            <div className="mb-4 flex justify-end gap-4">
                                <SelectLevelsModal
                                    ...
                                    onItemSelect={(level) => {
                                        appendLevel({
                                            levelId: level.id,
                                            levelRequiredPoint: level.conditionalPoint || 0,
                            ...
                            <SelectLevelsTable
                                ...
                                handleRemoveLevel={(level) => {
                                    const index = campaignGroupFields.findIndex((g) => g.groupId === level.id);
                                    if (index >= 0) {
                                        removeGroup(index);
                                    }
                                }}
