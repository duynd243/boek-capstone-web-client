...
const IssuerAuthorModal: React.FC<Props> = ({
    maxWidth,
    action,
    isOpen,
    onClose,
    ...
    const createAuthorMutation = useMutation(
        (payload: CreateAuthorParams) => authorService.createIssuerAuthor(payload),
        commonMutationOptions
    );
        ...
        >
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="space-y-3 py-4 px-5">
                    {/* <Modal.FormLabel label={action === AuthorModalMode.UPDATE ? "Ảnh đại diện" :""} />
                    {action === AuthorModalMode.UPDATE && (
                            <Controller
                            name="previewFile"
                            control={control}
...
export default memo(IssuerAuthorModal);
