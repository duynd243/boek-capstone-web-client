...
const Page: NextPageWithLayout = () => {
    ...
    const { data: dashboardSummary } = useQuery(
        ["issuer_dashboard_summary"],
        dashboardService.getIssuerDashboardSummary
    );
    ...
    const { data: campaignRevenue } = useQuery(
        ["issuer_campaign_revenue", campaignRevenueParams],
        () => dashboardService.getCampaignRevenueByIssuer(campaignRevenueParams)
    );
    ...
    const { data: customerSpending } = useQuery(
        ["issuer_customer_spending", customerSpendingParams],
        () =>
            dashboardService.getCustomerSpendingByIssuer(customerSpendingParams)
    );
                            ...
                            >
                                {(campaignRevenue?.models?.[0]?.data || [])?.map((item) => (
                                    <DropdownItem
                                        onClick={() => {
                                            if(item?.data?.id) {
                                                router.push(`/issuer/dashboard/campaigns/${item?.data?.id}`)
                                            }
                                        }}
                                        key={item?.data?.id}
